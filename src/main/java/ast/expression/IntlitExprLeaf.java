...
public final class IntlitExprLeaf implements ExprAST {
  ...
  public void checkType(ErrorHandler errorHandler, SymbolTable symbolTable, Type type) {
    if (!type.getTypeToken().equals(TypeToken.INT) && !type.getTypeToken().equals(TypeToken.ANY)) {
      errorHandler.report(
          new IncompatibleTypeError(
              String.valueOf(value), line, charPosition, type.getTypeToken(), TypeToken.INT));
    }
  }
