...
import error.semanticError.FunctionParamError;
import error.semanticError.FunctionUndefinedError;
    ...
    if (!type.getTypeToken().equals(typeToken) && !typeToken.equals(TypeToken.ANY)) {
      errorHandler.report(
          new IncompatibleTypeError(
              "call " + identifier.getText(), line, charPosition, type.getTypeToken(), typeToken));
    }
    setType(type);
  }
